...
import (
	...
	"github.com/openshift/openshift-azure/pkg/common"
)
...
func (r *ReconcileNamespace) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	if !common.RestrictedNamespace(request.Name) {
		...
		// - customer-view
		logrus.Printf("Reconciling Namespace %s/%s", request.Namespace, request.Name)
		...
		if ns.Status.Phase == corev1.NamespaceTerminating {
			logrus.Printf("namespace is being deleted %v", request.Namespace)
			return reconcile.Result{}, nil
		}
		...
		for _, rbname := range common.RoleBindingNames {
			...
			if err != nil {
				if kerrors.IsNotFound(err) {
					...
					logrus.Printf("Creating rolebinding: %#v", nrb)
